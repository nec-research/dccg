...
train_dataset = MNIST_Addition(
    torchvision.datasets.MNIST(root=path, train=True, download=True, transform=transform),
    path+'train_data.txt')
test_dataset = MNIST_Addition(
    torchvision.datasets.MNIST(root=path, train=False, download=True, transform=transform),
    path+'test_data.txt')
...
def train(num_epochs, alpha, tau, beta):  
    ...
    i = 1
    running_loss = 0.0
    loss_list, f1_list = list(), list()
    for epoch in range(num_epochs):
        for data in train_loader:
            optimizer.zero_grad()
            input1, input2, labels = data
            input1, input2, labels = Variable(input1.to(device)), Variable(input2.to(device)), Variable(labels.to(device))
            ...
            if i % log_period == 0:
                f1, acc = test_MNIST(net, tau, test_loader, len(test_dataset), device)
                inter_loss = running_loss / (log_period*batch_size)
                form = 'epoch {:2}  step {}  alpha {:.3}  beta {:.3}  loss {:.4}  acc {:.4}  f1 {:.4}'
                logger.info(form.format(epoch, i, alpha, beta, inter_loss, acc, f1))
                running_loss = 0
            i += 1
